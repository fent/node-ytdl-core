...
exports.getLikes = info => {
  try {
    ...
    let like = buttons.find(b => b.segmentedLikeDislikeButtonViewModel)?.segmentedLikeDislikeButtonViewModel.likeButtonViewModel.likeButtonViewModel;
    return parseInt(like.toggleButtonViewModel.toggleButtonViewModel.defaultButtonViewModel.buttonViewModel.title) | 0;    
  } catch (err) {
    return null;
  }
};
...
exports.getDislikes = info => {
  try {
    ...
    let video = contents.find(r => r.videoPrimaryInfoRenderer);
    let buttons = video.videoPrimaryInfoRenderer.videoActions.menuRenderer.topLevelButtons;
    let dislike = buttons.find(b => b.segmentedLikeDislikeButtonViewModel)?.segmentedLikeDislikeButtonViewModel.dislikeButtonViewModel.dislikeButtonViewModel;
    return parseInt(dislike.toggleButtonViewModel.toggleButtonViewModel.defaultButtonViewModel.buttonViewModel.title) | 0;    
  } catch (err) {
    return null;
  }
};

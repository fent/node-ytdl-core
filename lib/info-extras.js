  ...
  try {
    ...
    for (let row of metadataRows) {
      if (row.metadataRowRenderer) {
        ...
        media[title + '_url'] = url.resolve(VIDEO_URL,
        ...
        for (let content of contents) {
          ...
          media.image = url.resolve(VIDEO_URL, media.thumbnails[0].url);
          switch (meta.style) {
            case 'RICH_METADATA_RENDERER_STYLE_BOX_ART': {
              ...
              media[type + '_url'] = url.resolve(VIDEO_URL,
...
exports.getAuthor = (info) => {
  ...
  let avatar, verified;
  if (v) try {
    avatar = v.videoSecondaryInfoRenderer.owner.videoOwnerRenderer.thumbnail.thumbnails[0].url;
  ...
  return {
    ...
    user_url: videoDetails.ownerProfileUrl,
  };
};

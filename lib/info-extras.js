  ...
  try {
    ...
    for (let row of metadataRows) {
      if (row.metadataRowRenderer) {
        ...
        // Using metadata with the title "song" to indicate new song data
        if (title === 'song') {
          pushMedia(medias, media);
        ...
        for (let { richMetadataRenderer } of boxArt) {
          ...
          // Using metadata with the type "game" to indicate new game data
...
function pushMedia(medias, media) {
  // If there is data in the meda object push onto the medias array

...
exports.getBasicInfo = (id, options, callback) => {
  ...
  .then((response) => {
      ...
      const unavailableMsg = util.between(response, '<div id="player-unavailable"', '>');
      if (unavailableMsg &&
          !/\bhid\b/.test(util.between(unavailableMsg, 'class="', '"'))) {
        // Ignore error about age restriction.
        if (!response.includes('<div id="watch7-player-age-gate-content"')) {
          return callback(Error(util.between(response,
      ...
      const additional = {
        ...
        author: extras.getAuthor(response),
  
        // Get the day the vid was published.
        published: extras.getPublished(response),
  
        // Get description.
        description: extras.getVideoDescription(response),
  
        // Get media info.
        media: extras.getVideoMedia(response),
  
        // Get related videos.
        related_videos: extras.getRelatedVideos(response),
  
        // Give the standard link to the video.
        video_url: VIDEO_URL + id,
      };
      const jsonStr = util.between(response, 'ytplayer.config = ', '</script>');
      let config;
      if (jsonStr) {
        ...
        .then((res) => {
          config = util.between(res, 't.setConfig({\'PLAYER_CONFIG\': ', /\}(,'|\}\);)/);
          return gotConfig(id, options, additional, config, true, callback)
        })
        .catch((err) => {
          callback(err)
        })

...
module.exports = function getInfo(link, opts, callback) {
  ...
  request(url, function(err, body) {
    ...
    if (jsonStr) {
      ...
      requestAndMergeMpd(id, config, function(err) {
        if (err) {
          return callback(new Error('failed getting embed info'));
        }
...
function requestAndMergeMpd(id, config, callback) {
  var infoUrl = getInfoUrl(id);
  //the dashmpd for a vivo always returns a 403
  //youtube-dl also ignores mpd download
  if (config.args.ptk === 'vevo') {
    callback();
  } else {
    if (!config.args.dashmpd) {
      request(infoUrl, function(err, body) {
        if (err) return callback(err);
        var escaped = querystring.parse(body);
        config.args.dashmpd = escaped.dashmpd;
        callback();
      });
    } else {
      callback();
...
function getInfoUrl(id) {
  //there is a type vevo but the mpd is 403
  var type = 'info';
  return format({
    protocol: 'https',
    host: INFO_HOST,

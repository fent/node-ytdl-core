...
const urlUtils = require('ytdl-core/lib/url-utils');
const extras = require('ytdl-core/lib/info-extras');
const sig = require('ytdl-core/lib/sig');
const Cache = require('ytdl-core/lib/cache');
...
const getYTCfg = body => {
  let cfg;
  let cfgPos = body.indexOf('ytcfg.set('); 
  while(cfgPos>=0) { // There are several cfg object blocks; the order of blocks and the order of properties can vary
    body = body.substr(cfgPos+10);
    try {
      cfg = JSON.parse(body.substr(0,body.indexOf(');')));    
      if (cfg.INNERTUBE_API_KEY) return { 
        key : cfg.INNERTUBE_API_KEY,
        context : cfg.INNERTUBE_CONTEXT,
        session_token : cfg.XSRF_TOKEN,
        logged_in : cfg.LOGGED_IN,
        id_token : cfg.ID_TOKEN
       };
    } catch(e) {
    }
    cfgPos = body.indexOf('ytcfg.set(');
  }
  return cfg;
}
...
const getWatchJSONURL = (id, options) => `${getWatchHTMLURL(id, options)}&has_verified=1&pbj=1`;
const getWatchJSONPage = async(id, options) => {
  ...
  const params = {
    'command': {
      clickTrackingParams: options.ytcfg.context.clickTracking.clickTrackingParams,
      commandMetadata: {
        webCommandMetadata: {
    ...
    'session_token': options.ytcfg.session_token
  }
  ...
  reqOptions.headers = Object.assign({
    ...
    'x-youtube-client-version': options.ytcfg.context.client.clientVersion,
    'x-youtube-identity-token':  options.ytcfg.id_token || exports.cookieCache.get(cookie || 'browser') || '',
  }, reqOptions.headers);
  ...
  const req = utils.exposedMiniget(jsonUrl, options, reqOptions);
...
const getWatchHTMLPage = async(id, options) => {
  ...
  if (!options.ytcfg) options.ytcfg = getYTCfg(body);
  return info;
};


const INNER_HOST = 'www.youtube.com';
const INNER_PATH = '/youtubei/v1/player';
const getVideoInfoInner = async(id, options) => {
  let info = { page: 'inner' };
  if (options.ytcfg) {
    const params = {
      context: options.ytcfg.context,
      videoId: id
    }
    params.context.user = {};
    const postData = JSON.stringify(params);
    const url = new URL(`https://${INNER_HOST}${INNER_PATH}`);
    url.searchParams.set('key', options.ytcfg.key);
    const postOptions = {...options.requestOptions};
    postOptions.headers = {...postOptions.headers};
    postOptions.method = 'POST';
    postOptions.headers['content-type'] = 'application/json';
    postOptions.headers['content-length'] = Buffer.byteLength(postData,'utf8');
    postOptions.headers['x-goog-visitor-id'] = options.ytcfg.context.client.visitorData;
    let req = utils.exposedMiniget(url.toString(), options, postOptions);
    req.once('request',(request) => {
      request.write(postData);
    });
    info.player_response = JSON.parse(await req.text());
  }
  return info;
};

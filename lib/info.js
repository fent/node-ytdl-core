...
exports.getBasicInfo = (id, options, callback) => {
  ...
  request(reqOptions, (err, res, body) => {
    ...
    const captchaForm = util.between(body, '<form action="/das_captcha" method="POST">', '</form>');
    if(captchaForm) {
      let siteKey = util.between(captchaForm, 'class="g-recaptcha" data-sitekey="', '"');
      let next = util.between(captchaForm, 'class="g-recaptcha" data-sitekey="', '"');
      let sessionToken = util.between(captchaForm, 'name="session_token" value="', '"');
      return callback(Error('reCAPTCHA form shown with `data-sitekey`: ' + siteKey + ', `next`: ' + next + ', `session_token`: ' + sessionToken));
    }
    ...
    if (jsonStr) {
      ...
      options.requestOptions.url = url;
      request(options.requestOptions, (err, res, body) => {
        if (err) return callback(err);
...
//   return formats;
// };
const parseFormats = (info, core_info) => {
  let formats = [];
  if (core_info.csi_page_type === 'embed' && core_info.player_response && core_info.player_response.streamingData) { 
      core_info.player_response.streamingData.formats.map(m => {
          let sp = '';
          let s = '';
          let url = '';
          if (m.cipher) {
              let cipherParse = m.cipher.split("&");
              cipherParse.map(cp => {
                  let fm = cp.split("=");
                  if (fm[0] === "s") s = decodeURIComponent(fm[1]);
                  if (fm[0] === "sp") sp = decodeURIComponent(fm[1]);
                  if (fm[0] === "url") url = decodeURIComponent(fm[1])
              });
          }
          formats.push({
              quality_label: m.qualityLabel,
              fps: (m.fps) ? m.fps : null,
              projection_type: m.projectionType,
              clen: m.contentLength,
              index: null,
              sp: (sp !== '') ? sp : null,
              xtags: '',
              type: m.mimeType,
...
const gotConfig = (id, options, additional, config, fromEmbed, callback) => {
  ...
  request(options.requestOptions, (err, res, body) => {
    ...
    info.formats = parseFormats(info, core_info);
...
const getDashManifest = (url, options, callback) => {
  ...
  options = options || {};
  options.requestOptions = options.requestOptions || {};
  options.requestOptions.url = urllib.resolve(VIDEO_URL, url);
  const req = request(options.requestOptions);
  if(req.setEncoding) {

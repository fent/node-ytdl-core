  ...
  util.parallel([
    function(callback) {
      ...
      myrequest(url, options.requestOptions, function(err, body) {
        ...
        relatedIds = getRelatedVideos(body);
    ...
    config.args.relatedIds = relatedIds;
...
function getRelatedVideos(body) {
  // Get the RELATED_PLAYER_ARGS from body then parse for the video Ids
  var related = body.match(/RELATED_PLAYER_ARGS['\s\t\n]*:[\s\t\n]*{[\s\t\n"]*rvs":"([^}]+)/);
  if (!related) {
    return [];
  }

  var re= /id=([a-zA-Z0-9_-]+)/gm;
  var m;
  var relatedIds = [];
  while ((m = re.exec(related[1])) !== null) {

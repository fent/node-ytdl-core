...
const parseFormats = (info) => {
  ...
  formats.forEach(f => {
    ...
    delete f.mimeType
    if (f.cipher) {
    let parsed = querystring.parse(f.cipher);
    let s = parsed.s, end = s[s.length - 1]
    s = s.slice(1).replace('=', end).replace(/.$/, "=");
    f.url = parsed.url + "&sig=" + s
    delete f.cipher
    }
    });
  }
...
const gotConfig = (id, options, additional, config, fromEmbed, callback) => {
  ...
  request(url, options.requestOptions, (err, res, body) => {
    ...
    if (!info.player_response.streamingData.formats && !info.player_response.streamingData.adaptiveFormats &&

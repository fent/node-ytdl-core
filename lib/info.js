  ...
  util.parallel([
    function(callback) {
      ...
      myrequest(url, options.requestOptions, function(err, body) {
        ...
        if (!relatedVideos) {
          relatedVideos = getRelatedVideos(body);
        }
    ...
    gotConfig(options, description, relatedVideos, config, dashmpd2, callback);
  });
};
 ...
 * @param {String} body
 */
function getRelatedVideos(body) {
  var jsonStr = util.between(body, 'yt.setConfig(', '</script>');
  var related = [];
  if (jsonStr) {
    try {
      ...
      if (config && config.RELATED_PLAYER_ARGS && config.RELATED_PLAYER_ARGS.rvs) {
        ...
        for (i in qStrs) {
          var q = querystring.parse(qStrs[i]);
          related.push({
            id: q.id,
            author: q.author,
            title: q.title,
 ...
 * @param {Function(Error, Object)} callback
 */
function gotConfig(options, description, relatedVideos, config, dashmpd2, callback) {
  ...
  info.relatedVideos = relatedVideos;

...
module.exports = function getInfo(link, options, callback) {
  ...
  myrequest(url, options.requestOptions, function(err, body) {
    ...
    var additional = {};
    // Get informations about the owner/uploader
    additional.owner = util.getOwner(body);
    // Get the day the vid was published
    additional.published = util.getPublished(body);
    // Get description from #eow-description.
    additional.description = util.getVideoDescription(body);
    // Get related vids
    additional.relatedVideos = util.getRelatedVideos(body);
...
function gotConfig(id, options, additional, config, callback) {
  var statusErr = checkStatusErr(config.args);
  ...
  myrequest(url, options.requestOptions, function(err, body) {
    ...
    var statusErr = checkStatusErr(info);
    if (statusErr) return callback(statusErr);
    ...
    //add the additional to the infos and keep it future proof
    info = Object.assign({}, info, additional);
 ...
 * @return {!Error}
 */
function checkStatusErr(info) {
  if (info.status === 'fail') {
    return new Error(info.errorcode && info.reason ?

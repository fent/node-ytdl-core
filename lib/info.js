...
var EMBED_URL = 'http://www.youtube.com/embed/';
var KEYS_TO_SPLIT = [
  'keywords',
  'fmt_list',
  'fexp',
...
module.exports = function getInfo(link, opts, callback) {
  ...
  var description = $('#eow-description').text();
    ...
    if (jsonStr) {
      parseJSON(opts, jsonStr, description, callback);
    } else {
 ...
 * @param {Function(Error, Object)} callback
 */
function parseJSON(opts, body, description, callback) {
  var jstream = new JStream();
  var ended = false;
  jstream.on('data', function(config) {
    ended = true;
    jstream.pause();
    gotConfig(opts, config, description, callback);
  });
 ...
 * @param {Function(Error, Object)} callback
 */
function gotConfig(opts, config, description, callback) {

...
exports.getBasicInfo = async (id, options) => {
  ...
  // return await gotConfig(id, options, info, body);

  if (!info.player) {
    // If the video page doesn't work, maybe because it has mature content.
    // and requires an account logged in to view, try the embed page.
    let embedUrl = EMBED_URL + id + '?' + params;
    let [, body] = await miniget.promise(embedUrl, options.requestOptions);
    let configStr = util.between(body, 't.setConfig({\'PLAYER_CONFIG\': ', /\}(,'|\}\);)/);
    let config;
    if (!configStr) {
...
const gotConfig = async (id, options, info, body) => {
  ...
  // require('fs').writeFileSync('moreinfo2.json', JSON.stringify(moreinfo, null, 2));
  return info;
};

...
const ytdl = (link, options) => {
  ...
  ytdl.getInfo(link, options, (err, info) => {
    ...
    downloadFromInfoCallback(stream, info, options, {firstTime: true});
  });
...
const downloadFromInfoCallback = (stream, info, options, options_request) => {
  ...
  let downloaded = options_request.downloaded || 0;
  const ondata = (chunk) => {
    downloaded += chunk.length;
    stream.emit('progress', chunk.length, downloaded, format.contentLength);
  };
  ...
  if (format.isHLS || format.isDashMPD) {
    ...
    request(format.url).once('response', (res) => {
      ...
      if (!format.contentLength) {
        format.contentLength = parseInt(res.headers['content-length'], 10);
      }
      if (options_request.firstTime) stream.emit('response', res)
    });
  ...
  if (typeof options_request.bytes !== 'undefined' && (format.contentLength > options_request.bytes)) {
  options_request.downloaded = downloaded
  options_request.firstTime = false

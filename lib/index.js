...
const handleFormatUrl = (url, preferAudio, cb) => {
  if(url.indexOf('manifest.googlevideo.com') !== -1) {
    request(url, (err, resp, body) => {
      if(!err) {
        parseString(body, {trim: true}, (err, result) => {
          if(!err) {
            try {
              result.MPD.Period[0].AdaptationSet.forEach(elm => {
                if(elm["$"].mimeType == "audio/mp4" && preferAudio) {
                  cb(elm.Representation[0].BaseURL[0]);
                  throw BreakException;
                } else if(elm["$"].mimeType == "video/mp4" && !preferAudio) {
                  cb(elm.Representation[0].BaseURL[0]);
                  throw BreakException;
                }
              });
...
const downloadFromInfoCallback = (stream, info, options) => {
  ...
  handleFormatUrl(format.url, false, (url) => {
    format.url = url;

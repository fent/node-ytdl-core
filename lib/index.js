...
const getMaxLength = (format, callback) => {
if (format.contentLength) return callback(format.contentLength)
else request(format.url).once('response', (res) => callback(res.headers['content-length']))
}

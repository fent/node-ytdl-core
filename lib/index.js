...
function doDownload(stream, url, options, tryCount, reconnectInfo) {
  ...
  req.on('error', function(err) {
    if (stream._isDestroyed || !myres) { return stream.emit('error', err); }
    stream.unpipe(myres);

    // Reconnect on any error after download starts.
  ...
  req.on('response', function(res) {
    ...
    res.on('data', function(d) {
      var downloaded = reconnectInfo.downloaded += d.length;
      stream.emit('progress', d.length, downloaded, reconnectInfo.total);
    });

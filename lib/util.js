...
exports.getOwner = function(body) {
	...
	if(ownerinfo != '') {
		var type = ownerinfo.match( new RegExp( '\/(user|channel)\/') )[1]
		var ref = ownerinfo.match( new RegExp('<a href="(\/'+type+'\/[\\w-]+)"') )[1]
		var id = ref.match( new RegExp('\/'+type+'\/([\\w-]+)') )[1]
		var avatar = ownerinfo.match( new RegExp('<img.+?(?=data-thumb)data-thumb="([^"]+)"[^>]+>') )[1]
		if(type == 'user') {
			var hiddenchannel_type = 'channel'
			var hiddenchannel_ref = ownerinfo.match( new RegExp('<a href="(\/'+hiddenchannel_type+'\/[\\w-]+)"') )[1]
			var hiddenchannel_id = hiddenchannel_ref.match( new RegExp('\/'+hiddenchannel_type+'\/([\\w-]+)') )[1]
			var hiddenchannel_name = ownerinfo.match( new RegExp('<a href="\/channel\/'+((type == 'user')?(hiddenchannel_id):(id))+'"[^>]+>(.+?(?=<\/a>))') )[1]
		}
		var name = (type == 'user') ? hiddenchannel_name : ownerinfo.match( new RegExp('<a href="\/channel\/'+id+'"[^>]+>(.+?(?=<\/a>))') )[1]
	}
	return {
		'type':type,
		'ref':ref,
		...
		'name':name,
		'hiddenchannel':{
			'type':hiddenchannel_type,
			'ref':hiddenchannel_ref,
			'id':hiddenchannel_id,
			'name':hiddenchannel_name
		}
	}
};

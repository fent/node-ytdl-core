 ...
 * @return {Object}
 */
exports.getOwner = function(body) {
  var ownerinfo = this.between(body,'<div id="watch-header" class="yt-card yt-card-has-padding">','<div id="watch-action-panels" class="watch-action-panels yt-uix-button-panel hid yt-card yt-card-has-padding">')
  var response = {};
  if(ownerinfo != '') {
    response.type = ownerinfo.match( new RegExp( '\/(user|channel)\/') )[1]
    response.ref = ownerinfo.match( new RegExp('<a href="(\/'+response.type+'\/[\\w-]+)"') )[1]
    response.id = response.ref.split('/')[2]
    response.avatar = ownerinfo.match( new RegExp('<img.+?(?=data-thumb)data-thumb="([^"]+)"[^>]+>') )[1]
    if(response.type == 'user') {
      response.hiddenchannel = {};
      response.hiddenchannel.type = 'channel'
      response.hiddenchannel.ref = ownerinfo.match( new RegExp('<a href="(\/'+response.hiddenchannel.type+'\/[\\w-]+)"') )[1]
      response.hiddenchannel.id = response.hiddenchannel.ref.split('/')[2]
      response.hiddenchannel.name = ownerinfo.match( new RegExp('<a href="\/channel\/'+((response.type == 'user')?(response.hiddenchannel.id):(response.id))+'"[^>]+>(.+?(?=<\/a>))') )[1]
    }
    response.name = (response.type == 'user') ? response.hiddenchannel.name : ownerinfo.match( new RegExp('<a href="\/channel\/'+response.id+'"[^>]+>(.+?(?=<\/a>))') )[1]
  }
  return response;
};
...
exports.getPublished = function(body) {
  return Date.parse( this.between(body,'<meta itemprop="datePublished" content="','">') )
};
...
exports.getRelatedVideos = function(body) {
  jsonStr = this.between(body, 'yt.setConfig(', ');')
  var related = [];
  if (jsonStr) {
    ...
    if (config && config.RELATED_PLAYER_ARGS && config.RELATED_PLAYER_ARGS.rvs) {
      var qStrs = config.RELATED_PLAYER_ARGS.rvs.split(',');
      if(qStrs.length > 0) {
        related = qStrs.map(function(relatedVideo){
		var q = qs.parse(relatedVideo)
		return {
          'id':q.id,
          'author':q.author,
          'title':q.title,

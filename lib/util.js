...
/**
 * Get video extras from html
 * 
 * @param {String} body
 * @return {Object}
 */
const extrasRegexp = /<h4 class="title">[\s\S]*?([\s\S]*?)<\/h4>[\s\S]*?<ul.*?class=".*?watch-info-tag-list\">[\s\S]*?<li>([\s\S]*?)<\/li>/g;
exports.getVideoExtras = (body) => {
  let extrainfo = exports.between(body,
    '<div id="watch-description-extras">',
    '<div id="watch-discussion" class="branded-page-box yt-card">');
  if (extrainfo === '') {
    return {};
  }
  extrainfo = new Entities().decode(extrainfo);

  const contentRegexp = /<a.*?>(.*?)<\/a>/;
  const extras = {};
  let match = extrasRegexp.exec(extrainfo,toString());
  while (match) {
    if (match[2].match(contentRegexp)) match[2] = contentRegexp.exec(match[2])[1];
    extras[match[1].trim().replace(/\s/g, '_').toLowerCase()] = match[2];
    match = extrasRegexp.exec(extrainfo);
  }
  return extras;
};
...
exports.getAuthor = (body) => {
  let ownerinfo = exports.between(body,
...
exports.getRelatedVideos = (body) => {
  let jsonStr = exports.between(body, '\'RELATED_PLAYER_ARGS\': {"rvs":', '},');
  try {

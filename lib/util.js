...
exports.filterFormats = function(formats, filter) {
  ...
  switch (filter) {
    ...
    case 'audioonly':
      ...
      break;
    case 'index':
      fn = function (format) { return format.index && format.init; };
      break;
...
/**
 * Get the segment corresponding to the given time by reading the index
 * 
 * @param {Object} index 
 * @param {number} time
 * @param {number} def the default return value if no segment 
 * @returns {number} offset
 */
exports.getSegmentAt = function (index, time) {
  // TODO: dichotomic search ? 
  if (index.segments.length === 0) return null
  var segment = null;
  var duration = 0;
  var i = 0;
  while (i < index.segments.length && duration < time) {
    segment = index.segments[i];
    i++;
    duration = index.segments[i].timestamp;
  }
  return segment
}

/**
 * Parse the ranges string in the format
 * 
 * @param {object} format 
 * @returns {object} ranges
 */
exports.parseRanges = function (format) {
  if (!format.index || !format.init) throw new Error('Format has no ranges')
  var m1 = format.init.split('-')
  var m2 = format.index.split('-')
  return {
    init: {
      start: parseInt(m1[0]),
      end: parseInt(m1[1])

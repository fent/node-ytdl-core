  ...
  switch (quality) {
    ...
    case 'highestaudio':
      ...
      for (let f of formats) {
        if (!format ||
          audioScore(f) > audioScore(format)) format = f;
      }
      break;
    ...
    case 'highestvideo':
      ...
      for (let f of formats) {
        if (!format ||
          getBitrate(f) > getBitrate(format)) format = f;
      }
      break;
    ...
    default:
      format = getDefaultFormat(quality, formats);
      break;
  }
...
const getDefaultFormat = (quality, formats) => {

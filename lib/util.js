...
exports.chooseFormat = (formats, options) => {
  ...
  switch (quality) {
    ...
    case 'highestaudio':
      var highest = formats[0];
      for (var format of formats) {
        if (format.audioBitrate > highest.audioBitrate
          || (format.audioBitrate === higher.audioBitrate && !format.encoding))
            highest = format;
      }
      format = highest;
      break;
    ...
    case 'highestvideo':
      var highest = formats[0];
      for (var format of formats) {
        if (format.bitrate.split('-')[0] > highest.bitrate.split('-')[0]
          || (format.bitrate.split('-')[0] === higher.bitrate.split('-')[0] && !format.audioEncoding))
            highest = format;
      }
      format = highest;
      break;

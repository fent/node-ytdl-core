...
exports.sortFormats = (a, b) => {
  ...
  const getBitrate = (c) => {
    if (c.bitrate) {
      let s = typeof c.bitrate === 'string' && c.bitrate.split('-');
...
exports.parseFormats = (info) => {
  ...
  if (info.player_response && info.player_response.streamingData && info.player_response.streamingData.formats) {
    ...
    formats.forEach(function (format) {
      if (format.cipher) {
        format.cipher = qs.parse(format.cipher)

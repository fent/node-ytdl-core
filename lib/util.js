  ...
  switch (quality) {
    ...
    case 'highestaudio':
      ...
      for (let f of formats) {
        if (f.audioBitrate > highest.audioBitrate
          || (f.audioBitrate === highest.audioBitrate && !f.encoding))
            format = f;
      }
      break;
    ...
    case 'highestvideo':
      function getBitrate(f) {
        let s = highest.bitrate.split('-')
        return parseFloat(s[s.length - 1], 10);
      }
      formats = exports.filterFormats(formats, 'video');
      format = formats[0];
      for (let f of formats) {
        if (getBitrate(f) > getBitrate(highest)
          || (getBitrate(f) === getBitrate(highest) && !f.audioEncoding))
            format = f;
      }
      break;

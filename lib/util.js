...
exports.getRandomIPv6 = block => {
  ...
  block = block.split('/')[0];
  if (!subnet || subnet > 128 || subnet < 24) throw new Error('Invalid IPv6 subnet'); // Other errors will be thrown by ip6 itself
  let ip;
  if (subnet < 64) block = ip6.randomSubnet(block, subnet, 64, 1, true)[0];
  if (subnet > 64) ip = ip6.randomSubnet(block, subnet, 128, 1, true)[0];
  else ip = ip6.randomSubnet(block, 64, 128, 1, true)[0];
  return ip;
};

 ...
 * @returns {number}
 */
const indexOf = (haystack, needle) => needle instanceof RegExp ?
...
exports.cutAfterJSON = mixedJson => {
  ...
  for (var i = 0; i < mixedJson.length; i++) {
    ...
    if (mixedJson[i - 1] === '\\') continue;

    // Cut at first semi-colon
    if (mixedJson[i] === ';') {
      break;
    }
  ...
  if (counter === 0) {
    // Return the cut JSON
    return mixedJson.substr(0, i);
  } else {
    throw new Error("can't cut unsupported JSON (uneven open and closing brackets");
  }
};

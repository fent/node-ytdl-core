...
exports.cutAfterJSON = function(mixedJson) {
  let counter = 0, pos = 0, isString, prevChar, open, close;
  ...
  if (!open) {
    throw new Error("can't cut unsupported JSON (need to begin with [ or { )");
  }
  for (let character of mixedJson) {
    pos++;
    if (!isString && character === '"' && prevChar !== '\\') {
      isString = true;
      prevChar = character;
      continue;
    }
    if (!isString) {
      ...
      if (counter <= 0) {
        return mixedJson.slice(0, pos);
      }
    } else if (character === '"' && prevChar !== '\\') {
      isString = null;
    }
    prevChar = character;
  }

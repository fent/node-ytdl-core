  ...
  let extractNCode = () => {
    ...
    if (functionName && functionName.length) {
      ...
      if (ndx >= 0) {
        let subBody = body.slice(ndx + functionStart.length);
        let functionBody = `var ${functionStart}${utils.cutAfterJSON(subBody)}`;
        functions.push(`${functionBody};${functionName}(ncode);`);
      }
    }
  };
  extractDecipher();
...
exports.setDownloadURL = (functions, format) => {
  let decipher = url => {
    ...
    const decipherFunction = new vm.Script(functions[0]);
    const context = vm.createContext({ sig: decodeURIComponent(args.s) });
    let components = new urllib.URL(decodeURIComponent(args.url));
    components.searchParams.set(args.sp ? args.sp : 'signature', decipherFunction.runInContext(context));
  ...
  let ncode = url => {
    ...
    const nTransformFunction = new vm.Script(functions[1]);
    const context = vm.createContext({ ncode: n });
    components.searchParams.set('n', nTransformFunction.runInContext(context));
    return components.toString();
  };
